    ...
    for i in range(2 * eps):
        for j in range(2 * eps):
            if (-1 < borderx + i < width) and (-1 < bordery + j < height):
                lst.append(arr[bordery + j, borderx + i])
...
def frame(I):
    global start, end
    w = complex(-math.sin(math.tau * I / 1000)*.75, math.cos(math.tau * I / 1000)*.75)

    img = Image.open(f"frames\\{I*2}.png")
    pixar = np.array(img)
    ...
    for _j in range(height):
        for _i in range(width):
            ...
            try:
                if abs(z) < 2:
                    col = mean_col(((z.real - X) * s + width/2, height/2 - (z.imag - Y) * s), reps, pixar)
                    canvas.putpixel((_i, _j), col)
            except OverflowError: ...

    canvas.save(f"newframes\\{I*2}.png")
